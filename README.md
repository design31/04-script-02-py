...
```
Почему-то в данном случае скрипт выдаёт полные имена файлов правильно только если скрипт лежит в папке репозитория. Если его переместить, то вывод будет содержать не путь к изменённому файлу в репозитории, а (путь к файлу скрипта)'+'(имя измененного файла). 
